...
int Compress::compress(const std::string& CompressName)
{
	...
	while (true)
	{
		...
		for (size_t i = 0; i < ReadSize; ++i)
		{
            std::string& code = m_informations[(unsigned char)ReadBuff[i]].m_code;
...
int Compress::uncompress(const std::string& CompressName)
{
	...
	for (size_t i = 0; i < LineCount; i++)
	{
		...
		else
		{
			...
			m_informations[(unsigned char)strcodeinfo[0]].m_count = count;//读每个字符出现的次数
		}
	}
	...
	Node cur = ht.GetRoot();
	CountType FileSize = cur->m_weight.m_count;//源文件一共有多少个字符
...
int main()
{
    std::cout << "法购房款拒对方就怕二级从登记哦" << std::endl;
    std::cout << "法购房款拒对方就怕二级从登记哦" << std::endl;
    std::cout << "法购房款拒对方就怕二级从登记哦" << std::endl;
    std::cout << "法购房款拒对方就怕二级从登记哦" << std::endl;
    std::cout << "法购房款拒对方就怕二级从登记哦" << std::endl;
